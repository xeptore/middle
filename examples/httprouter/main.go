...
func main() {
	...
	router.Handler("GET", "/path-that-ignores-handler-error", middle.Chain3(m1, m2, m3).Then(handler))
	router.Handler("GET", "/path-that-handles-handler-error", middle.Chain3(m1, m2, m3).Then(handler).Finally(unexpectedErrHandle))
